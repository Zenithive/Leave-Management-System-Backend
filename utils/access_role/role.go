...
func Manager(role string, message string) error {
...
// IsAdminLike returns true for SUPERADMIN and ADMIN — roles with full data access.
func IsAdminLike(role string) bool {
	return role == constant.ROLE_SUPER_ADMIN || role == constant.ROLE_ADMIN
}

// IsAdminOrHR returns true for ADMIN and HR — roles that cannot touch SUPERADMIN.
func IsAdminOrHR(role string) bool {
	return role == constant.ROLE_ADMIN || role == constant.ROLE_HR
}
