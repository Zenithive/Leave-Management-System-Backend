...
func (h *HandlerFunc) CreateCategory(c *gin.Context) {
	...
	if err != nil {
		pkg.RespondWithError(c, http.StatusUnauthorized, err.Error())
		return
	}
	...
	if err := models.Validate.Struct(&req); err != nil {
		pkg.RespondWithError(c, http.StatusBadRequest, "validation error: "+err.Error())
		return
	}
...
func (h *HandlerFunc) GetAllCategory(c *gin.Context) {
	...
	filters := pkg.GetFilterParams(c, pkg.CategorySortFields)
	...
	if err != nil {
		pkg.RespondWithError(c, http.StatusInternalServerError, "failed to get categories: "+err.Error())
		return
	}

	response := gin.H{"message": "success", "categories": data}
	if pagination.PageSize > 0 {
		response["pagination"] = pkg.CalculatePaginationResponse(pagination.Page, pagination.PageSize, total)
	}
...
func (h *HandlerFunc) UpdateCategory(c *gin.Context) {
	...
	if err != nil {
		pkg.RespondWithError(c, http.StatusBadRequest, "invalid category ID")
		return
	}
	empID, err := common.GetEmployeeId(c)
	if err != nil {
...
func (h *HandlerFunc) GetAllEquipment(c *gin.Context) {
	...
	pagination := pkg.GetPaginationParams(c)
	filters := pkg.GetFilterParams(c, pkg.EquipmentSortFields)
	...
	if err != nil {
		pkg.RespondWithError(c, http.StatusInternalServerError, "failed to get equipment: "+err.Error())
		return
	}
	if data == nil {
...
func (h *HandlerFunc) GetEquipmentByCategory(c *gin.Context) {
	...
	if categoryIDStr == "" {
		pkg.RespondWithError(c, http.StatusBadRequest, "category_id query parameter is required")
		return
	}
...
func (h *HandlerFunc) UpdateEquipment(c *gin.Context) {
	...
	if err != nil {
		pkg.RespondWithError(c, http.StatusBadRequest, "invalid equipment ID")
		return
	}
	empID, err := common.GetEmployeeId(c)
	if err != nil {
...
func (h *HandlerFunc) GetAllAssignedEquipment(c *gin.Context) {
	...
	pagination := pkg.GetPaginationParams(c)
	filters := pkg.GetFilterParams(c, pkg.AssignmentSortFields)
