...
func (p *WithdrawProcessor) Process(ctx context.Context, tx *sqlx.Tx, lctx *LeaveActionContext) error {
	...
	// 3. Skip any sibling APPROVED stages at the same stage_no (safety, mirrors approve)
	...
	// 5. If higher APPROVED stages still exist → intermediate, not fully withdrawn yet
	...
	// 6. All stages settled → fully withdrawn
	if err := lctx.CommRepo.UpdateLeaveStatusWithApprover(
	...
	// 7. Restore balance ONLY for the final (highest stage_no) approver — non-early only
